...
def get_address(str):
    street_val = r"\s*\b(ул|проспек\w+|ст|жилой район|бульв|б-р|мкр|п\.г\.т|мкр\Wн|кв\Wл|квартал|проезд|бульвар|аллея|пер|наб|шоссе|линия|просп|пр\Wкт|переулок|улиц\w+|пр)\b\.?"
    addr_val = r"\s*\b(г|д|р\.п|дер|пос|г\.о|м\Wр\Wн|МО г\.п|городской|сельский|" \
               r"сельское поселение|село|городское поселение|городской округ|населенный пункт|" \
    ...
    end_val = r"(,|(?<![дгрс])\.|$|площад\w+|общей|»|\(|;|с земельным|и земельный) *"
    reg_exp = r'(:?по\b|:|\.|,|в\b|с адресом\s*-)(?P<address>(:?' \
              r'(' \
              r'\s*\b(Россия|РФ)\b\.?|' \
              r'{addr_val}[^,:]+?|' \
    ...
    if match:
        address = re.sub(r'\s*({end_val})+$'.format(end_val=end_val), '', match["address"])
        return address
    return ""
